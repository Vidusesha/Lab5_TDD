...
contract("StandardERC20", (accounts) => {
  ...
  it("test increaseAllowance()", async () => {
    const spenderAllowance1 = await standardERC20Instance.allowance.call(
        receipient1,
        spender
      );
      
      const increaseAllowance = await standardERC20Instance.increaseallowance(
        spender,
        spenderAmount,
        { from: receipient1 }
      );
  
      truffleAssert.eventEmitted(increaseAllowance, "Approval", (obj) => {
        return (
          obj.owner === receipient1 &&
          obj.spender === spender &&
          new BigNumber(obj.value).isEqualTo(spenderAllowance1.add(spenderAmount))
        );
      });

      const spenderAllowance2 = await standardERC20Instance.allowance.call(
        receipient1,
        spender
      );
    
    const expectedSpenderAllowance = web3.utils
        .toBN(spenderAllowance1)
        .add(spenderAmount);
    assert(
        new BigNumber(spenderAllowance2).isEqualTo(
            new BigNumber(expectedSpenderAllowance)
        ),
        "The spender's allowance is not as expected"
    );
    });
    ...
    it("test decreaseAllowance()", async () => {
      const spenderAllowance1 = await standardERC20Instance.allowance.call(
        ...
        const decreaseAllowance = await standardERC20Instance.decreaseallowance(
          spender,
          spenderAmount,
          { from: receipient1 }
        );
        ...
        truffleAssert.eventEmitted(increaseAllowance, "Approval", (obj) => {
          return (
            obj.owner === receipient1 &&
            obj.spender === spender &&
            new BigNumber(obj.value).isEqualTo(spenderAllowance1.sub(spenderAmount))
          );
        });
